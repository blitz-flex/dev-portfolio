      ...
      <div className="relative mt-12 max-w-5xl mx-auto">
        ...
        <div className="space-y-8">
          {experiences.map((exp, index) => (
            <div key={index} className="group relative grid md:grid-cols-[180px_auto_1fr] lg:grid-cols-[220px_auto_1fr] gap-6 md:gap-10 p-4 md:p-8 rounded-3xl transition-all duration-500 hover:bg-black/[0.02] dark:hover:bg-white/[0.02]">

              {/* Left Column: Date */}
              <div className="text-left md:text-right pt-1 md:pt-4">
                <span className="inline-block px-5 py-2 rounded-full bg-brand-primary/5 dark:bg-brand-primary/10 text-brand-primary text-xs sm:text-sm font-mono font-bold tracking-widest border border-brand-primary/10 transition-all duration-300 group-hover:bg-brand-primary group-hover:text-white group-hover:border-transparent">
                  {exp.date}
                </span>
              </div>
              ...
              <div className="hidden md:flex flex-col items-center relative z-10 w-8">
                ...
                <div className="w-[1.5px] h-full bg-gradient-to-b from-brand-primary/20 via-brand-primary/10 to-transparent absolute top-0 -bottom-8 group-last:h-[40px] transition-all duration-500 group-hover:via-brand-primary/40"></div>
                {/* The glowing dot */}
                <div className="w-4 h-4 rounded-full bg-surface-light dark:bg-surface-dark border-[3px] border-brand-primary/30 group-hover:border-brand-primary group-hover:scale-125 transition-all duration-500 will-change-transform mt-7 relative z-20 shadow-[0_0_15px_rgba(99,102,241,0)] group-hover:shadow-[0_0_20px_rgba(99,102,241,0.4)]"></div>
              </div>

              {/* Right Column: Content */}
              <div className="text-left pt-1 md:pt-3 relative z-10 transition-transform duration-500 group-hover:translate-x-1">
                <h3 className="text-xl md:text-2xl font-bold text-text-light dark:text-text-dark mb-2 flex flex-col sm:flex-row sm:items-center gap-2 transition-colors">
                  ...
                  <a href={exp.companyUrl} className="text-brand-primary hover:text-brand-secondary transition-colors inline-flex items-center gap-1.5" target="_blank" rel="noopener noreferrer">
                    {exp.company}
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                  </a>
                </h3>

                <p className="mt-4 text-text-light-muted dark:text-text-dark-muted leading-relaxed font-normal text-[1.05rem] max-w-3xl opacity-80 group-hover:opacity-100 transition-opacity">
                  {exp.description}
                </p>

                <div className="flex flex-wrap gap-2 mt-6">
                  {exp.technologies.map((tech, i) => (
                    <span key={i} className="tech-chip group-hover:border-brand-primary/30 transition-all duration-300">
                      {tech}
                    </span>
                  ))}
                </div>
